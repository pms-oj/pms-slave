...
# * original: https://github.com/ioi-2017/tps/blob/master/samples/Communication/prize/scripts/templates/run.judge.sh
...
touch "${grader_in}"
cat > "${manager_in}"
"${sandbox}/exec_man.sh" "${g2m}" "${m2g}" < "${manager_in}" > "${manager_out}" 2> "${manager_err}" &
manager_pid=$!
